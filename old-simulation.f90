...
call position(N,L,rmin ,x, y, z, R)
...
!!!!!!!!!!!! computing the pair correlation function g(r) !!!!!!!
  ! choose reference particle rand, calculate g(r)= V/N *(n(r)/4pi*r**2 dr)= N_particles at distance r/5'pi r**2 dr *density)
 !!!  at each r average over configurztions set
!!! range rmin to rmax


!!!! average V(r) per particle , g(r), 
...
Subroutine position(N,L,rmin ,x, y, z, R)
integer, intent(in) :: N
double precision , intent(in) :: L, rmin
double precision , intent(out) :: x(N), y(N), z(N), R(N,N)
...
if (R(i,j) < rmin) then
!write(*,*) "initial position: R is found <rmin, goto applied"
goto 10
end if
